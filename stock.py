...
def combine_historical_and_forecast(data: pd.DataFrame, forecast_df: pd.DataFrame, hist_label="Historical", fc_label="Forecast") -> pd.DataFrame:
    """Combine historical and forecast data for a single chart."""
    hist_data = data.reset_index()[["Date", "Close"]].copy()
    hist_data["Price"] = hist_data["Close"].round(2)
    hist_data["Type"] = hist_label
    hist_data.drop(columns=["Close"], inplace=True)
    
    fc_data = forecast_df[["Date", "forecast"]].copy()
    fc_data["Price"] = fc_data["forecast"].round(2)
    fc_data["Type"] = fc_label
...
def display_about():
    ...
    st.sidebar.info(
        "StockGPT is a comprehensive stock analysis and forecasting tool. It integrates historical data, news sentiment, technical indicators, "
        "and multiple forecasting models to deliver actionable insights."
    )
...
def main():
    ...
    with tabs[0]:
        ...
        st.markdown(f"<div class='header'>{ticker} Company Overview</div>", unsafe_allow_html=True)
        ...
        try:
            ...
            forecast_result = pd.DataFrame({"forecast": np.zeros(forecast_days),
        ...
        combined_data = combine_historical_and_forecast(data, forecast_result)
        fig_combined = px.line(combined_data, x="Date", y="Price", color="Type", title=f"{ticker} Historical vs. Forecast")
    ...
    with tabs[2]:
        ...
        fc_chart_data = best_result_adj.melt(id_vars="Date", value_vars=["forecast", "lower", "upper"],
    ...
    with tabs[4]:
        ...
        **Market Analysis:**
        ...
        - Technical indicators like RSI and MACD provide additional context.
        - Broader economic and political events also influence market trends.
        
        **Recommendations:**
        - Consider buying if sentiment and technical indicators are favorable.
        ...
        if st.button("Get Answer"):
            if "increase" in question.lower():
                st.write("Stocks may increase if positive sentiment and strong fundamentals persist.")
